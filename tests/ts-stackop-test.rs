...
mod tests {
    ...
    fn test_ts_many_in_current_stack() {
        let mut ts = TS::new();
        ts.many_in_current_stack(3, Value::from(42.0).unwrap());
    ...
    fn test_ts_many_in_stack() {
        ...
        ts.ensure_stack("A".to_string());
        ts.ensure_stack("B".to_string());
        ts_many_in_stack(&mut ts, 3, "A".to_string(), Value::from(42.0).unwrap());
